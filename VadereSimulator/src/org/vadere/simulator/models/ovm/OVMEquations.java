...
public class OVMEquations extends AbstractModelEquations<Car> implements IAsyncComputable {
  ...
  private void computeSingleCarParallel(
      Car currentCar, int carIdInArray, double t, double[] x, double[] xdot) {

    int fCI = -1;
    Car nearestCar = null;

    List<Car> neighbors =
        topography.getSpatialMap(Car.class).getObjects(currentCar.getPosition(), sightDistance);
    if (topography.hasTeleporter()) {
      Vector2D tpoint = topography.getTeleporter().getTeleporterShift();
      List<Car> ghostCars =
          topography
              .getSpatialMap(Car.class)
              .getObjects(currentCar.getPosition().add(tpoint), sightDistance);
    ...
    for (int j = 0; j < neighbors.size(); j++) {
      ...
      if (nearestCar == null) {
        ...
        if (car != currentCar && isInFront(car, currentCar) && isInFront(nearestCar, car)) {
          nearestCar = car;
          fCI = j;
        }
      }
  ...
  private boolean isInFront(Car car, Car currentCar) {
    if (this.attributesOVM.isIgnoreOtherCars()) return false;
   ...
   * @param index
   */
  private void computeSingleCar(
      Car currentCar, Car frontCar, double t, double[] x, double[] xdot, int index, int fCI) {
    ...
    if (currentCar.hasNextTarget()) {
      ...
      VPoint closest =
          new Vector2D(
              target.getShape().getBounds2D().getCenterX(),
              target.getShape().getBounds2D().getCenterY());
      double localspeed = mySpeed;
      Vector2D direction =
          new Vector2D(closest.add(myPos.scalarMultiply(-1))).normalize(localspeed);
    ...
    if (frontCar == null) {
      ...
      speed[0] =
          sensitivity * ovFunction(0.0, newTargetX, currentCar.getFreeFlowSpeed())
              - sensitivity * mySpeed;
    } else {
      speed[0] =
          sensitivity
                  * ovFunction(
                      0.0,
                      myPos.distance(frontCar.getPosition())
                          - currentCar.getAttributes().getLength() * 2,
                      currentCar.getFreeFlowSpeed())
              - sensitivity * myVelocity.getLength();
    }
    speed[1] = 0;
  ...
  @Override
  /** Dimension per Car 4 parameters: position (x,y), velocity (x,y) */
