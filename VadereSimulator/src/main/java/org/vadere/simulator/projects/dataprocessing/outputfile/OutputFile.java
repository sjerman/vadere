 ...
 *
 * <p>This class knows all the data processors of which the data should be saved. It writes the data
...
public abstract class OutputFile<K extends DataKey<K>> {
  ...
  // Note: Only header information from data keys are written in this, therefore, these are the
  // indices
  // Each processor attached to this processor itself attaches also headers, but they are not listed
  // in this
  // attribute.
  ...
  public static final String headerProcSep = "-";
  public static final String headerNameAdd =
      headerProcSep + "PID?"; // the # is replaced with the processor id
  ...
  private void printMetaData() {
    // use '#' symbol for comment -- the Java-style comment '//' is not appropriate because it
    // requires two
    // characters
    String md =
        "#IDXCOL="
            + dataIndices.length
            + ",DATACOL="
            + (getEntireHeader().size() - dataIndices.length)
            + ","
            + "SEP=\'"
            + DataProcessingJsonManager.DEFAULT_SEPARATOR
            + "\'";
  ...
  public List<String> getEntireHeader() {
    ...
    if (!isAddedProcessors) {
      throw new RuntimeException(
          "Asking for headers, but processors were not " + "initialized yet.");
    }
  ...
  private List<String> uniqueHeaderNames() {
    ...
    for (DataProcessor dataProcessor : dataProcessors) {
      ...
      for (String singleHeader : allProcHeaders) {
        ...
        String adaptedColumnName =
            OutputFile.addHeaderProcInfo(singleHeader, dataProcessor.getId());
      ...
      for (String el : list) {
        if (headers.contains(el)) {
          isNameMangleDetected =
              true; // conflict found: stop collecting and name make every header unique
          break mainloop;
        } else {
          headers.addLast(el);
        }
  ...
  private List<String> composeLine(
      String[] keyFieldArray,
  ...
  List<String> addProcessorToLine(
      final List<String> fields, Function<DataProcessor, Stream<String>> valueFields) {
    final List<String> processorFields =
        dataProcessors.stream().flatMap(valueFields).collect(Collectors.toList());
