...
public class JsonConverter {
  ...
  public static Scenario deserializeScenarioRunManager(String json)
  ...
  public static ModelDefinition deserializeModelDefinition(String json) throws IOException {
    ...
    return new ModelDefinition(
        mainModelString,
        StateJsonConverter.deserializeAttributesListFromNode(node.get("attributesModel")));
  }

  public static Scenario deserializeScenarioRunManagerFromNode(JsonNode node)
      throws IOException, IllegalArgumentException {
    ...
    AttributesSimulation attributesSimulation =
        StateJsonConverter.deserializeAttributesSimulationFromNode(
            scenarioNode.get(AttributesSimulation.JSON_KEY));
    AttributesPsychology attributesPsychology =
        StateJsonConverter.deserializeAttributesPsychologyFromNode(
            scenarioNode.get(AttributesPsychology.JSON_KEY));

    JsonNode attributesModelNode = scenarioNode.get("attributesModel");
    String mainModel =
        (!scenarioNode.has(StateJsonConverter.MAIN_MODEL_KEY)
                || scenarioNode.get(StateJsonConverter.MAIN_MODEL_KEY).isNull())
            ? null
            : scenarioNode.get(StateJsonConverter.MAIN_MODEL_KEY).asText();
    List<Attributes> attributesModel =
        StateJsonConverter.deserializeAttributesListFromNode(attributesModelNode);
    Topography topography =
        StateJsonConverter.deserializeTopographyFromNode(scenarioNode.get("topography"));
    StimulusInfoStore stimulusInfoStore =
        StateJsonConverter.deserializeStimuliFromArrayNode(scenarioNode);
    ...
    scenarioRunManager.setDataProcessingJsonManager(
        DataProcessingJsonManager.deserializeFromNode(
            rootNode.get(DataProcessingJsonManager.DATAPROCCESSING_KEY)));
    scenarioRunManager.saveChanges();

    return scenarioRunManager;
  }

  // used in hasUnsavedChanges, TODO [priority=high] [task=bugfix] check if commitHashIncluded can
  // always be false
  ...
  public static String serializeScenarioRunManager(
      Scenario scenarioRunManager, boolean commitHashIncluded) throws IOException {
    return StateJsonConverter.writeValueAsString(
        serializeScenarioRunManagerToNode(scenarioRunManager, commitHashIncluded));
  }

  public static JsonNode serializeScenarioRunManagerToNode(
      Scenario scenarioRunManager, boolean commitHashIncluded) {
    ...
    rootNode.set(
        DataProcessingJsonManager.DATAPROCCESSING_KEY,
        scenarioRunManager.getDataProcessingJsonManager().serializeToNode());
    rootNode.set(StateJsonConverter.SCENARIO_KEY, serializeVadereNode(scenarioStore));
    return rootNode;
  }

  private static void serializeMeta(
      ObjectNode node, boolean commitHashIncluded, ScenarioStore scenarioStore) {
    ...
    if (commitHashIncluded) node.put("commithash", Version.getVersionControlCommitHash());
  }
  ...
  private static ObjectNode serializeVadereNode(ScenarioStore scenarioStore) {
    ...
    ObjectNode attributesModelNode =
        StateJsonConverter.serializeAttributesModelToNode(scenarioStore.getAttributesList());
    vadereNode.set("attributesModel", attributesModelNode);

    vadereNode.set(
        AttributesSimulation.JSON_KEY,
        StateJsonConverter.convertValue(scenarioStore.getAttributesSimulation(), JsonNode.class));

    ObjectNode psychologyNode =
        StateJsonConverter.serializeAttributesPsychologyToNode(
            scenarioStore.getAttributesPsychology());
    vadereNode.set(AttributesPsychology.JSON_KEY, psychologyNode);

    ObjectNode topographyNode =
        StateJsonConverter.serializeTopographyToNode(scenarioStore.getTopography());
    vadereNode.set("topography", topographyNode);

    // We get a complete tree here and not only a node. Therefore, use "setAll()" instead of
    // "set()".
    ObjectNode eventNode =
        StateJsonConverter.serializeStimuliToNode(scenarioStore.getStimulusInfoStore());
  ...
  public static ScenarioStore cloneScenarioStore(ScenarioStore scenarioStore) throws IOException {
    JsonNode attributesSimulationNode =
        StateJsonConverter.convertValue(scenarioStore.getAttributesSimulation(), JsonNode.class);

    ObjectNode attributesPsychologyNode =
        StateJsonConverter.serializeAttributesPsychologyToNode(
            scenarioStore.getAttributesPsychology());
    ObjectNode attributesModelNode =
        StateJsonConverter.serializeAttributesModelToNode(scenarioStore.getAttributesList());
    ...
    return new ScenarioStore(
        scenarioStore.getName(),
        scenarioStore.getDescription(),
        scenarioStore.getMainModel(),
        StateJsonConverter.deserializeAttributesListFromNode(attributesModelNode),
