...
/**
 * This processor requires the true and found solution (both point and function value). The solution
 * are stored in OptimizationMetric and have to be set in StepCircleOptimizer.
 *
 * <p>Note: currently only the Nelder Mead and PedestrianOSM support this feature, but it should be
 * easy to generalize this to other optimizer or more general pedestrian.
 */
@DataProcessorClass()
public class PedestrianMetricOptimizationProcessor
    extends DataProcessor<EventtimePedestrianIdKey, OptimizationMetric> {
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    for (Pedestrian pedestrian : pedestrians) {
      ArrayList<OptimizationMetric> pedestrianMetrics =
          ((PedestrianOSM) pedestrian).getOptimizationMetricElements();

      if (pedestrianMetrics == null) {
        throw new RuntimeException(
            "Pedestrian OptimizationMetric is null. This means that there the "
        ...
        for (OptimizationMetric singleMetric : pedestrianMetrics) {
          putValue(
              new EventtimePedestrianIdKey(singleMetric.getSimTime(), singleMetric.getPedId()),
              singleMetric);
        }
  ...
  public void postLoop(final SimulationState state) {
    if (!this.receivedValues) {
      throw new RuntimeException(
          "PedestrianMetricOptimizationProcessor received no values. This can be because"
