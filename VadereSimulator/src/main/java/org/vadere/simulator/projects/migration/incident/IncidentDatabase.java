...
public class IncidentDatabase {
  ...
  private IncidentDatabase() {
    ...
    addIncident(
        NOT_A_RELEASE,
        new RelocationIncident(
            "attributesPedestrian", path("vadere"), path("vadere", "topography")));

    addIncident(NOT_A_RELEASE, new DeletionIncident(path("vadere", "topography", "pedestrians")));

    addIncident(
        NOT_A_RELEASE,
        new RenameInArrayIncident(
    ...
    for (String oldName : LookupTables.version0to1_ModelRenaming.keySet()) {
      ...
      addIncident(
          NOT_A_RELEASE, new RenameIncident(path("vadere", "attributesModel", oldName), newName));
    }
    ...
    addIncident(
        NOT_A_RELEASE,
        new MoveSpawnDelayIntoDistributionParametersIncident()); // requested by Jakob
