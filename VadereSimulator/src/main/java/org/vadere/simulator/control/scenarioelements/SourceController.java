...
public abstract class SourceController extends ScenarioElementController
    implements ControllerEventProvider<Agent, SourceController> {
  ...
  protected boolean testFreeSpace(
      final VShape freeSpace, final List<VShape> blockPedestrianShapes) {
    VShape freeSpaceCA =
        new VCircle(
            ((VCircle) freeSpace).getCenter(), ((VCircle) freeSpace).getRadius() - BUFFER_CA);
    final VShape freeSpaceModelSpecific =
        spawnerAttributes.isEventPositionGridCA() ? freeSpaceCA : freeSpace;

    boolean pedOverlap =
        blockPedestrianShapes.stream().noneMatch(shape -> shape.intersects(freeSpaceModelSpecific));
    boolean obstOverlap =
        this.getTopography()
            .getObstacles()
            .stream()
            .map(Obstacle::getShape)
            .noneMatch(shape -> shape.intersects(freeSpaceModelSpecific));
  ...
  protected abstract void determineNumberOfSpawnsAndNextEvent(double simTimeInSec);
  ...
  protected void createNextEvent() {
    ...
    if (newTimeOfNextEvent < timeOfNextEvent) { // TODO mit Herr Lehmberg reden wg. < vs <=
      ...
      if (distribution instanceof MixedDistribution) {
        distributionName =
            ((MixedDistribution) distribution).getCurrentDistribution().getClass().getSimpleName();
      } else {
        distributionName = distribution.getClass().getSimpleName();
      }
      throw new RuntimeException(
          "Bug: newTimeOfNextEvent is smaller or equal to timeOfNextEvent. The selected "
              + "distribution "
              + distributionName
              + " has a bug. "
  ...
  protected void addNewAgentToScenario(final VPoint position, double simTimeInSec) {
    ...
    // TODO [priority=high] [task=refactoring] this is bad programming. Why is the target list added
    // later?
    ...
    // TODO [priority=high] [task=refactoring] why only if he has no targets? because the
  ...
  private DynamicElement createDynamicElement(final VPoint position) {
    ...
    if (elementAttribute == null) {
      result =
          (Agent)
              dynamicElementFactory.createElement(
                  position, AttributesAgent.ID_NOT_SET, Pedestrian.class);
    } else if (elementAttribute instanceof AttributesAgent) {
      result =
          (Agent)
              dynamicElementFactory.createElement(
                  position, AttributesAgent.ID_NOT_SET, elementAttribute, Pedestrian.class);
    } else {
      throw new IllegalArgumentException(
          "The controller's source has an unsupported element type: " + elementAttribute);
    }
