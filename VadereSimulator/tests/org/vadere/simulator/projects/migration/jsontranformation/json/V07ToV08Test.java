...
public class V07ToV08Test extends JsonTransformationTest {
  ...
  public void testTyp1() throws MigrationException {
    ...
    for (String type : processorTypes) {
      ArrayList<JsonNode> processor = getProcessorsByType(jsonNew, type);
      for (JsonNode p : processor) {
        assertThat(
            p,
            fieldChanged(
                "attributes/", "measurementArea", "measurementAreaId", JsonNode::isNumber));
    ...
    for (String type : processorTypes) {
      ...
      for (JsonNode p : processor) {
        ...
        assertThat(
            p,
            fieldChanged(
                "attributes/", "voronoiArea", "voronoiMeasurementAreaId", JsonNode::isNumber));

        int measurementAreaId = pathMustExist(p, "attributes/measurementAreaId").asInt();
        int voronoiMeasurementAreaId =
            pathMustExist(p, "attributes/voronoiMeasurementAreaId").asInt();
  ...
  public void testTyp4() throws MigrationException {
    ...
    for (String type : processorTypes) {
      ...
      for (JsonNode p : processor) {
        assertThat(
            p, fieldChanged("attributes/", "waitingArea", "waitingAreaId", JsonNode::isNumber));
  ...
  //            assertThat(jsonNew, measurementAreaExists(path(processor,
  // "attributes/measurementAreaId").asInt()));
  //
