...
public class SimulationCommandHandler extends CommandHandler<SimulationVar> {
  ...
  private Pair<Double, Set<Integer>>
      allPrevious; // time at witch the given ids were send over traci
  private Pair<Double, List<String>>
      departedCache; // time at witch the given ids were send over traci
  private Pair<Double, List<String>>
      arrivedCache; // time at witch the given ids were send over traci
  ...
  public TraCIGetResponse responseOK(TraCIDataType responseDataType, Object responseData) {
    return responseOK(
        ...
        TraCICmd.RESPONSE_GET_SIMULATION_VALUE);
  }
  ...
  @SimulationHandler(
    cmd = TraCICmd.GET_SIMULATION_VALUE,
    var = SimulationVar.DATA_PROCESSOR,
    name = "getDataProcessorValue",
    ignoreElementId = false
  )
  public TraCICommand process_getDataProcessorValue(
    ...
    remoteManager.accessState(
        (manager, state) -> {
          ...
          var processor =
              state.getControllerProvider().getProcessorManager().getProcessor(processorId);
          if (processor instanceof CompoundObjectProvider) {
            ...
            cmd.setResponse(
                responseERR(
                    SimulationVar.DATA_PROCESSOR,
                    String.format(
                        "No processor found with id %d or processor does not provide CompoundObject",
                        processorId)));
          }
        });
    return cmd;
  ...
  @SimulationHandler(
    cmd = TraCICmd.GET_SIMULATION_VALUE,
    var = SimulationVar.NET_BOUNDING_BOX,
    name = "getNetworkBound",
  ...
  public TraCICommand process_getSimTime(TraCIGetCommand cmd, RemoteManager remoteManager) {
    ...
    remoteManager.accessState(
        (manager, state) -> {
          ...
          cmd.setResponse(
              responseOK(
                  SimulationVar.TIME.type, time.setScale(1, RoundingMode.HALF_UP).doubleValue()));
        });
  ...
  @SimulationHandler(
    cmd = TraCICmd.SET_SIMULATION_STATE,
    var = SimulationVar.EXTERNAL_INPUT_INIT,
    name = "init_control",
    ignoreElementId = true
  )
  public TraCICommand process_init_control(TraCISetCommand cmd, RemoteManager remoteManager) {
    ...
    try {
      ...
      remoteManager.accessState(
          (manager, state) -> {
            boolean isUsePsychologyLayer =
                state.getScenarioStore().getAttributesPsychology().isUsePsychologyLayer();
            if (!isUsePsychologyLayer) {
              cmd.setErr(
                  "Psychology layer must be active. Set isUsePsychologyLayer: true in *.scenario file.");
            }
          });
  ...
  @SimulationHandler(
    cmd = TraCICmd.SET_SIMULATION_STATE,
    var = SimulationVar.EXTERNAL_INPUT,
    name = "apply_control",
    ignoreElementId = true
  )
      ...
      remoteManager.accessState(
          (manager, state) -> {
            try {
              ...
              StimulusInfoStore stimulusInfoStore =
                  manager
                      .getRemoteSimulationRun()
                      .getStimulusController()
                      .getScenarioStore()
  ...
  public TraCICommand process_getCacheHash(TraCIGetCommand rawCmd, RemoteManager remoteManager) {
    ...
    try {
      ...
      rawCmd.setResponse(
          responseERR(SimulationVar.CACHE_HASH, "Ill formatted TraCIGetCacheHashCommand"));
    }
    return rawCmd;
  }
  ...
  @SimulationHandler(
    cmd = TraCICmd.GET_SIMULATION_VALUE,
    var = SimulationVar.DEPARTED_PEDESTRIAN_IDS,
    name = "getDepartedPedestrianId",
    dataTypeStr = "ArrayList<String>",
    ignoreElementId = true
  )
  public TraCICommand process_getDepartedPedestrianIds(
      TraCIGetCommand cmd, RemoteManager remoteManager) {

    calcArrivedDeparted(remoteManager);
    cmd.setResponse(
        responseOK(SimulationVar.DEPARTED_PEDESTRIAN_IDS.type, departedCache.getSecond()));
    return cmd;
  }

  @SimulationHandler(
    cmd = TraCICmd.GET_SIMULATION_VALUE,
    var = SimulationVar.ARRIVED_PEDESTRIAN_PEDESTRIAN_IDS,
    name = "getArrivedPedestrianIds",
    dataTypeStr = "ArrayList<String>",
    ignoreElementId = true
  )
  public TraCICommand process_getArrivedPedestrianIds(
      TraCIGetCommand cmd, RemoteManager remoteManager) {

    calcArrivedDeparted(remoteManager);
    cmd.setResponse(
        responseOK(SimulationVar.DEPARTED_PEDESTRIAN_IDS.type, arrivedCache.getSecond()));
    return cmd;
  }
          ...
          if (allPrevious.getFirst().equals(state.getSimTimeInSec())) {
            ...
            departedCache =
                Pair.create(
                    ...
                    departed.stream().map(i -> Integer.toString(i)).collect(Collectors.toList()));
            arrivedCache =
                Pair.create(
                    state.getSimTimeInSec(),
                    arrived.stream().map(i -> Integer.toString(i)).collect(Collectors.toList()));
          }
        });
  }
  ...
  @SimulationHandler(
    cmd = TraCICmd.GET_SIMULATION_VALUE,
    var = SimulationVar.POSITION_CONVERSION,
    name = "getPositionConversion",
    dataTypeStr = "ArrayList<String>",
    ignoreElementId = true
  )
  ...
  @SimulationHandler(
    cmd = TraCICmd.GET_SIMULATION_VALUE,
    var = SimulationVar.COORD_REF,
    name = "getCoordinateReference",
    dataTypeStr = "ArrayList<String>",
    ignoreElementId = true
  )
  public TraCICommand process_GetCoordinateReference(
      TraCIGetCommand cmd, RemoteManager remoteManager) {
    remoteManager.accessState(
        (manager, state) -> {
          ReferenceCoordinateSystem coord =
          ...
          if (coord != null) {
            coord.initialize();
            CompoundObject o =
                CoordRef.asCompoundObject(coord.getEpsgCode(), coord.getTranslation());
  ...
  public TraCICommand processValueSub(TraCICommand rawCmd, RemoteManager remoteManager) {
    return processValueSub(
        rawCmd,
        remoteManager,
        this::processGet,
        TraCICmd.GET_SIMULATION_VALUE,
        TraCICmd.RESPONSE_SUB_SIMULATION_VALUE);
  }
  ...
  public TraCICommand processGet(TraCICommand rawCmd, RemoteManager remoteManager) {
    ...
    logger.tracef(
        ...
        m.getName(), cmd.getTraCICmd().logShort(), var.toString());
    ...
    try {
      remoteManager.accessState(
          (manager, state) -> {
            ...
            cmd.setResponse(
                responseOK(SimulationVar.OUTPUT_DIR.type, resultDir.toAbsolutePath().toString()));
          });
    } catch (TraCICommandCreationException ee) {
  ...
  public TraCICommand process_getObstacles(TraCIGetCommand cmd, RemoteManager remoteManager) {
    try {
      remoteManager.accessState(
          (manager, state) -> {
            List<ScenarioElement> obstacles =
                state
                    .getTopography()
                    .getAllScenarioElements()
                    .stream()
            ...
            for (ScenarioElement scenarioElement : obstacles) {
              ...
              if (scenarioElement.getShape() instanceof VRectangle) {
                ...
                cmd.setResponse(
                    responseERR(SimulationVar.OBSTACLES, "Obstacle must be of type polygon."));
              }
              obstacleShapes.put(scenarioElement.getId(), polygon);
            }
            cmd.setResponse(
                responseOK(SimulationVar.OBSTACLES.type, obstacleShapes.toJSONString()));
          });
    } catch (TraCICommandCreationException ee) {
