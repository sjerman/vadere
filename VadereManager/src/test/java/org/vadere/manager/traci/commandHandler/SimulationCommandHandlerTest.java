...
public class SimulationCommandHandlerTest extends CommandHandlerTest {
  ...
  public void process_getCacheHash() throws IOException {
    ...
    // response ok //
    String basePath = "src/test/resources/testProject001/scenarios";
    ...
    TraCIGetCommand cmd =
        (TraCIGetCommand)
            getFirstCommand(
                TraCIGetCacheHashCommand.build(
    ...
    // Also: The scenario is deserialized and serialised internally, setting the lineseperators
    // platform dependent
  ...
  public void process_getSimTime() {
    ...
    TraCIGetCommand cmd =
        (TraCIGetCommand)
            getFirstCommand(TraCIGetCommand.build(TraCICmd.GET_SIMULATION_VALUE, varID, elementID));
    RemoteManager rm =
        new TestRemoteManager() {
          @Override
          protected void mockIt() {
