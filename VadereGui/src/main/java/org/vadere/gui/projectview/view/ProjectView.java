...
public class ProjectView extends JFrame
    implements ProjectFinishedListener,
   ...
   *
   * <p>TODO [priority=medium] [task=refactoring] do the actions have to be stored in member
  ...
  private final Set<Action> projectSpecificActions =
      new HashSet<>(); // actions that should only be enabled, when a project is loaded
  ...
  public void postScenarioRun(final Scenario cloneScenario, final int scenarioLeft) {
    EventQueue.invokeLater(
        () -> {
          ...
          progressPanel.setData(
              Messages.getString("ProgressPanelWorking.text"),
              100 * doneScenariosCount / totalScenariosCount);
  ...
  public void error(final Scenario scenario, final int scenarioLefts, final Throwable throwable) {
    EventQueue.invokeLater(
        () -> {
          ...
          new Thread(
                  () -> {
                    // Use the causing exception (if available) to get a more meaningful error
                    // message.
                    ...
                    IOUtils.errorBox(
                        errorText,
                        Messages.getString("ProjectView.simulationRunErrorDialog.title"));
                  })
              .start();
        });
  }
  ...
  public void postRefresh() {
    EventQueue.invokeLater(
        () -> {
          if (!scenariosRunning) outputTable.setEnabled(true);
        });
  }
  ...
  private void openLastUsedProject(final ProjectViewModel model) {
    String lastUsedProjectPath = VadereConfig.getConfig().getString("History.lastUsedProject");
  ...
  private void buildKeyboardShortcuts(
      ActionPauseScenario pauseScenarioAction, Action interruptScenariosAction) {
  ...
  private void buildMenuBar(
      ActionCloseApplication closeApplicationAction, ActionAddScenario addScenarioAction) {
    ...
    setAcceleratorFromLocalizedShortcut(
        createProjectAction, "ProjectView.mntmNewTestProject.shortcut");
    ...
    setAcceleratorFromLocalizedShortcut(
        loadProjectAction, "ProjectView.mntmLoadTestProject.shortcut");
    ...
    Action changeNameAction =
        new ActionRenameProject(Messages.getString("ProjectView.mntmChangeName.text"), model);
    ...
    setAcceleratorFromLocalizedShortcut(
        saveProjectAction, "ProjectView.mntmSaveTestProject.shortcut");
    ...
    Action saveProjectAsAction =
        new ActionSaveAsProject(Messages.getString("ProjectView.mntmSaveAs.text"), model);
    setAcceleratorFromLocalizedShortcut(
        saveProjectAsAction,
        "ProjectView.mntmSaveAs.shortcut",
    ...
    mnFile.addSeparator();
    JCheckBoxMenuItem showResultDialogMenu =
        new JCheckBoxMenuItem(
            Messages.getString("ProjectView.mntmSimulationResult.text"),
            null,
            model.getShowSimulationResultDialog());
    Action showResultDialogMenuAction =
        new ShowResultDialogAction(
            Messages.getString("ProjectView.mntmSimulationResult.text"),
            model,
    ...
    JCheckBoxMenuItem toggleScenarioCheckerDialogMenu =
        new JCheckBoxMenuItem(
            Messages.getString("ProjectView.btnToggleScenarioChecker.text"),
            null,
            model.getShowSimulationResultDialog());
    Action toggleScenarioCheckerMenuAction =
        new ToggleScenarioManagerAction(
            Messages.getString("ProjectView.btnToggleScenarioChecker.text"),
            model,
            toggleScenarioCheckerDialogMenu);
    ...
    Action generateScenarioFromOutputAction =
        new ActionGenerateScenarioFromOutputFile(
    ...
    setAcceleratorFromLocalizedShortcut(
        generateScenarioFromOutputAction, "ProjectView.mntmGenerateScenario.shortcut");
    ...
    Action showAboutAction =
        new ActionShowAboutDialog(Messages.getString("ProjectView.mntmAbout.text"));
    JMenuItem mntmAbout = new JMenuItem(showAboutAction);

    mnHelp.add(mntmAbout);

    JMenu mntmLanguageChoiceMenu =
        new JMenu(Messages.getString("ProjectView.mntmLanguageChoiceMenu.text"));
    mnHelp.add(mntmLanguageChoiceMenu);
    JRadioButtonMenuItem mntmEnglishLocale =
        new JRadioButtonMenuItem(
            new AbstractAction(Messages.getString("ProjectView.mntmEnglishLocale.text")) {
    ...
    JRadioButtonMenuItem mntmGermanLocale =
        new JRadioButtonMenuItem(
            new AbstractAction(Messages.getString("ProjectView.mntmGermanLocale.text")) {
              private static final long serialVersionUID = 1L;
    ...
    if (Messages.languageIsGerman()) mntmGermanLocale.setSelected(true);
    else mntmEnglishLocale.setSelected(true);

    JMenuItem mntmReapplyMigration =
        new JMenuItem(
            new AbstractAction(Messages.getString("ProjectView.mntmReapplyMigration.text")) {
              private static final long serialVersionUID = 1L;

              @Override
              public void actionPerformed(ActionEvent e) {
                (new ActionLoadProject(
                        Messages.getString("ProjectView.mntmLoadTestProject.text"), model))
                    .loadProject(true);
              }
            });
  ...
  private void setAcceleratorFromLocalizedShortcut(Action action, String localizedShortcutKey) {
    setAcceleratorFromLocalizedShortcut(
        action, localizedShortcutKey, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask());
  }

  private void setAcceleratorFromLocalizedShortcut(
      Action action, String localizedShortcutKey, int mask) {
  ...
  private void buildContentPane() {
    ...
    ((BasicSplitPaneUI) mainSplitPanel.getUI())
        .getDivider()
        .addMouseListener(
            new MouseAdapter() {
              @Override
  ...
  private void buildScenarioTable(OutputTableRenderer outputTableRenderer) {
    ...
    scenarioTable
        ...
        .addListSelectionListener(
            new TableSelectionListener(scenarioTable) {
              @Override
              public void onSelect(ListSelectionEvent e) {
                ...
                logger.info(
                    String.format("selected scenario '%s'", bundle.getScenario().getName()));
  ...
  private void buildOutputTable(OutputTableRenderer outputTableRenderer) {
    ...
    outputTable
        .getSelectionModel()
        .addListSelectionListener(
            new TableSelectionListener(outputTable) {
              @Override
              public void onSelect(ListSelectionEvent e) {
                ...
                try {
                  ...
                  logger.info(
                      String.format("selected output file '%s'", bundle.getDirectory().getName()));
              ...
              private void loadScenarioIntoGui(OutputBundle bundle) throws IOException {
                ...
                Optional<File> trajectoryFile =
                    IOUtils.getFirstFile(bundle.getDirectory(), IOUtils.TRAJECTORY_FILE_EXTENSION);

                File[] txtFiles = IOUtils.getFileList(bundle.getDirectory(), ".txt");
                List<String> tableNames =
                    new ArrayList<>(
                        Arrays.asList(ContactData.TABLE_NAME, TableAerosolCloudData.TABLE_NAME));
                ...
                if (trajectoryFile.isPresent()) {
                  if (optionalPostVisFiles.size() > 0) {
                    scenarioJPanel.loadOutputFileForPostVis(
                        trajectoryFile.get(), optionalPostVisFiles, scenarioRM);
                  } else {
                  ...
                  logger.error(
                      "could not find trajectory file in : "
  ...
  private void buildOutputTablePopup() {
    ActionDeleteOutputDirectories deleteOutputFileAction =
        new ActionDeleteOutputDirectories(
    ...
    outputListPopupMenu.add(
        new JMenuItem(
            new ActionRenameOutputFile(Messages.getString("ProjectView.mntmRename.text"), model)));
    outputListPopupMenu.add(
        new JMenuItem(
            new ActionOutputToScenario(
                Messages.getString("ProjectView.mntmOutputToSceneario.text"), model)));
    outputListPopupMenu.add(
        new JMenuItem(
            new ActionRunOutput(Messages.getString("ProjectView.mntmRunOutput.text"), model)));
    outputListPopupMenu.add(
        new JMenuItem(
            new ActionOpenInExplorer(
                Messages.getString("ProjectView.OpenInExplorer.text"), model)));
                    ...
                    for (File file : children) {
                      String name =
                          file.isDirectory()
                              ...
                              : "---*" + file.getName();
                      copyPath.add(
                          new JMenuItem(new ActionToClipboard(name, file.getAbsolutePath())));
                    }
                  }
  ...
  private void buildScenarioTablePopup(ActionAddScenario addScenarioAction) {
    ActionDeleteScenarios deleteScenariosAction =
        new ActionDeleteScenarios(
            Messages.getString("ProjectView.mntmDelete.text"), model, scenarioTable);
    ActionRunSelectedScenarios runSelectedScenarios =
        new ActionRunSelectedScenarios(
            Messages.getString("ProjectView.mntmRunSelectedTests.text"), model, scenarioTable);
    ActionRunRepeatedlyScenarios runRepeatedlyScenarios =
        new ActionRunRepeatedlyScenarios(
            Messages.getString("ProjectView.mntmRunRepeatedlyTests.text"),
            model,
            scenarioTable,
            n_repetitions);
    ActionSeeDiscardChanges seeDiscardChangesAction =
        new ActionSeeDiscardChanges(
    ...
    scenarioListPopupMenu.add(
        new JMenuItem(
            new ActionEditScenarioDescription(
                Messages.getString("ActionEditScenarioDescription.menu.title"), model)));
    scenarioListPopupMenu.add(
        new JMenuItem(
  ...
  private void buildToolBar() {
    ...
    runAllScenariosAction.putValue(
    ...
    ActionRunSelectedScenarios runSelectedScenarios =
        new ActionRunSelectedScenarios(
            Messages.getString("ProjectView.mntmRunSelectedTests.text"), model, scenarioTable);
    runSelectedScenarios.putValue(
        Action.SHORT_DESCRIPTION, Messages.getString("ProjectView.btnRunSelectedTest.toolTipText"));
    runSelectedScenarios.putValue(
        Action.LARGE_ICON_KEY,
    ...
    ActionRunRepeatedlyScenarios runRepeatedlyScenarios =
        new ActionRunRepeatedlyScenarios(
            ...
            scenarioTable,
            n_repetitions);
    runRepeatedlyScenarios.putValue(
    ...
    Action interruptScenariosAction =
        new ActionInterruptScenarios(
            Messages.getString("ProjectView.btnStopRunningTests.text"), model);
    interruptScenariosAction.putValue(
        Action.LARGE_ICON_KEY,
    ...
    ActionResumeNormalSpeed resumeNormalSpeedAction =
        new ActionResumeNormalSpeed(
            Messages.getString("ProjectView.btnResumeNormalSpeed.text"), model);
    resumeNormalSpeedAction.putValue(
        Action.LARGE_ICON_KEY,
        new ImageIcon(ProjectView.class.getResource("/icons/greenarrow_right_small.png")));
    ...
    toolBar
        .getInputMap()
        .put(
            KeyStroke.getKeyStroke(Messages.getString("ProjectView.pauseTests.shortcut").charAt(0)),
            "pauseTests");
  ...
  public void updateRecentProjectsMenu() {
    ...
    java.util.List<String> recentProjectPaths =
        VadereConfig.getConfig()
            .getList(String.class, "History.recentProjects", Collections.EMPTY_LIST);
        ...
        if (model.getCurrentProjectPath() != null) {
          if (!model
              ...
              .equals(
                  Paths.get(path)
                      .getParent()
                      .toString())) { // when project loaded, hide that from recent list
