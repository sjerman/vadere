...
/**
 * The table panelModel used in the table that displays {@link
 * org.vadere.simulator.projects.Scenario}s.
 */
public class VadereScenarioTableModel
    extends VadereTableModelSorted<VadereScenarioTableModel.VadereDisplay> {
  ...
  VadereScenarioTableModel() {
    super(
        new String[] {
          ...
          Messages.getString("ProjectView.testTable.state.text")
        },
        0,
  ...
  public synchronized int indexOfRow(final Scenario value) {
    for (int i = 0; i < getRowCount(); i++)
      if (((Scenario) getValueAt(i, 0)).getName().equals(value.getName())) return i;
  ...
  public void insertRow(int row, VadereDisplay value) {
    insertRow(row, new Object[] {value.scenarioRM, value.state.toString()});
  }
