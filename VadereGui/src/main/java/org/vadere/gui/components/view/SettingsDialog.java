...
public class SettingsDialog extends JDialog {
  ...
  /**
   * The content is usually placed in every second cell (row or column). Therefore, increment by two
   * to jump to the next content cell.
   */
  ...
  /**
   * The settings dialog consists of several rows: 1. Color settings for static scenario elements
   * like sources or targets. 2. Color settings for agents. 3. Other settings consisting of: *
   * Visibility checkboxes to control whether scenario elements should be visible or not. * Snapshot
   * options (for PNG, SVG and TikZ export). 4. Post-visualization options (also called additional
   * options). 5. A row containing a close button.
   */
  public void initComponents() {
    ...
    scrollPane.setPreferredSize(
        new Dimension(
            mainPanel.getPreferredSize().width + 10,
            Math.min(
  ...
  private JPanel createMainPanel() {
    ...
    FormLayout mainLayout =
        new FormLayout(
            "5dlu, [300dlu,pref,600dlu], 5dlu", // cols
  ...
  private void initColorSettingsPane(JLayeredPane colorSettingsPane) {
    colorSettingsPane.setBorder(
        BorderFactory.createTitledBorder(Messages.getString("SettingsDialog.colors.border.text")));

    FormLayout colorSettingsLayout =
        new FormLayout(
            "5dlu, pref:grow, 2dlu, pref, 2dlu, pref, 2dlu, pref, 2dlu, pref, 5dlu", // col
    ...
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblObstacle.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblTarget.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblSource.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblStair.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblDensityColor.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblAbsorbingAreaColor.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblTargetChanger.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblAerosolCloudColor.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblDropletCloudColor.text") + ":"),
        cc.xy(column, row += NEXT_CELL));
  ...
  private void createColorCanvasesAndChangeButtonsOnPane(JLayeredPane colorSettingsPane) {
    ...
    bObstColor.addActionListener(
        new ActionSetObstacleColor("Set Obstacle Color", model, pObstacleColor));
    ...
    final JButton bStairsColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    bStairsColor.addActionListener(
        new ActionSetStairsColor("Set Stairs Color", model, pStairsColor));
    colorSettingsPane.add(pStairsColor, cc.xy(column4, row += NEXT_CELL));
    colorSettingsPane.add(bStairsColor, cc.xy(column5, row));

    final JButton bDensityColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    bDensityColor.addActionListener(
        new ActionSetDensityColor("Set Density Color", model, pDensityColor));
    colorSettingsPane.add(pDensityColor, cc.xy(column4, row += NEXT_CELL));
    colorSettingsPane.add(bDensityColor, cc.xy(column5, row));

    final JButton bAbsorbingAreaColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    bAbsorbingAreaColor.addActionListener(
        new ActionSetAbsorbingAreaColor("Set Absorbing Area Color", model, pAbsorbingAreaColor));
    ...
    final JButton bTargetChangerColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    bTargetChangerColor.addActionListener(
        new ActionSetTargetChangerColor("Set Target Changer Color", model, pTargetChangerColor));
    ...
    final JButton bAerosolCloudColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    bAerosolCloudColor.addActionListener(
        new ActionSetAerosolCloudColor("Set Aerosol Cloud Color", model, pAerosolCloudColor));
    ...
    final SpinnerNumberModel sModelAerosolCloudAlpha =
        new SpinnerNumberModel(model.config.getAerosolCloudAlphaMax(), 0, 255, 1);
    spinnerAerosolCloudAlpha.setModel(sModelAerosolCloudAlpha);
    spinnerAerosolCloudAlpha.addChangeListener(
        e -> {
          model.config.setAerosolCloudAlphaMax((int) sModelAerosolCloudAlpha.getValue());
          model.notifyObservers();
        });
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblAerosolCloudAlpha.text") + ":"),
        cc.xy(column2, row));
    colorSettingsPane.add(spinnerAerosolCloudAlpha, cc.xy(column3, row));

    final JButton bDropletCloudColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    bDropletCloudColor.addActionListener(
        new ActionSetDropletsColor("Set Droplet Cloud Color", model, pDropletCloudColor));
  ...
  private void initAgentColorSettingsPane(JLayeredPane colorSettingsPane) {
    ...
    colorSettingsPane.setBorder(
        BorderFactory.createTitledBorder(
            Messages.getString("SettingsDialog.pedcolors.border.text")));

    JRadioButton rbTargetColoring =
        createRadioButtonWithListener(
            AgentColoring.TARGET,
            Messages.getString("SettingsDialog.lblTargetColoring.text") + ":");
    JRadioButton rbRandomColoring =
        createRadioButtonWithListener(
            AgentColoring.RANDOM, Messages.getString("SettingsDialog.chbUseRandomColors.text"));
    JRadioButton rbGroupColoring =
        createRadioButtonWithListener(
            AgentColoring.GROUP, Messages.getString("SettingsDialog.chbGroupColors.text"));
    JRadioButton rbSelfCategoryColoring =
        createRadioButtonWithListener(
            AgentColoring.SELF_CATEGORY,
            Messages.getString("SettingsDialog.lblSelfCategoryColoring.text") + ":");
    JRadioButton rbHealthStatusColoring =
        createRadioButtonWithListener(
            AgentColoring.HEALTH_STATUS,
            Messages.getString("SettingsDialog.lblHealthStatusColoring.text") + ":");
    JRadioButton rbInformationColoring =
        createRadioButtonWithListener(
            AgentColoring.INFORMATION_STATE,
            Messages.getString("SettingsDialog.lblInformationColoring.text") + ":");
    ...
    final JPanel pTargetColor = new JPanel();
    final JButton bChangeTargetColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    final JPanel pPedestrianColorNoTarget = new JPanel();
    final JButton bChangePedestrianColorNoTarget =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));

    initColoringByTargetId(
        cbTargetIds,
        pTargetColor,
        bChangeTargetColor,
        bChangePedestrianColorNoTarget,
        pPedestrianColorNoTarget);

    JComboBox<SelfCategory> cbSelfCategories = createSelfCategoriesComboBox();
    final JPanel pSelfCategoryColor = new JPanel();
    final JButton bChangeSelfCategoryColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));
    ...
    final JButton bChangePedestrianColorLowerExposure =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));

    final JPanel pPedestrianColorUpperExposure = new JPanel();
    final JButton bChangePedestrianColorUpperExposure =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));

    final JPanel pPedestrianColorInfectious = new JPanel();
    final JButton bChangePedestrianColorInfectious =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));

    initColoringByHealthStatus(
        bChangePedestrianColorLowerExposure,
        pPedestrianColorLowerExposure,
        bChangePedestrianColorUpperExposure,
        pPedestrianColorUpperExposure,
        bChangePedestrianColorInfectious,
    ...
    final SpinnerNumberModel sModelLowerExposure =
        new SpinnerNumberModel(
            model.config.getLowerVisualizedExposure(),
            0,
            model.config.getUpperVisualizedExposure(),
            1);
    spinnerLowerExposure.setModel(sModelLowerExposure);
    spinnerLowerExposure.addChangeListener(
        e -> {
    ...
    final SpinnerNumberModel sModelUpperExposure =
        new SpinnerNumberModel(
            model.config.getUpperVisualizedExposure(),
            model.config.getLowerVisualizedExposure(),
            null,
    ...
    spinnerUpperExposure.addChangeListener(
        e -> {
    ...
    final JButton bChangeInformationStateColor =
        new JButton(Messages.getString("SettingsDialog.btnEditColor.text"));

    initColoringByInformationState(
        cbInformationStates, pInformationStateColor, bChangeInformationStateColor);
    ...
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblPedestrianNoTarget.text") + ":"),
        cc.xy(column2, row += NEXT_CELL));
    ...
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblPedestrianLowerExposure.text") + ":"),
        cc.xy(column2, row));
    ...
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblPedestrianUpperExposure.text") + ":"),
        cc.xy(column2, row += NEXT_CELL));
    ...
    colorSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblPedestrianInfectious.text") + ":"),
        cc.xy(column2, row += NEXT_CELL));
    ...
    // Evacuation time and criteria coloring comes in the next row see
    // "postvisualization/.../SettingsDialog.java".
  }
  ...
  private void initColoringByTargetId(
      JComboBox<Integer> cbTargetIds,
      JPanel pTargetColor,
      JButton bChangeTargetColor,
      JButton bChangePedestrianColorNoTarget,
      JPanel pPedestrianColorNoTarget) {
    ...
    pTargetColor.setBackground(
        colorByTargetId.orElseGet(() -> model.config.getPedestrianDefaultColor()));
    pTargetColor.setPreferredSize(new Dimension(COLOR_JPANEL_WIDTH, 20));

    // When user changes a color, save it in the model.
    bChangeTargetColor.addActionListener(
        new ActionSetPedestrianColor("Set Pedestrian Color", model, pTargetColor, cbTargetIds));

    // Retrieve configured color from "model" or use default color.
    cbTargetIds.addActionListener(
        e -> {
          ...
          pTargetColor.setBackground(
              colorByTarget.orElseGet(() -> model.config.getPedestrianDefaultColor()));
        });

    Optional<Color> notTargetPedCol = config.getColorByTargetId((-1));
    pPedestrianColorNoTarget.setBackground(
        notTargetPedCol.orElseGet(() -> model.config.getPedestrianDefaultColor()));
    pPedestrianColorNoTarget.setPreferredSize(new Dimension(COLOR_JPANEL_WIDTH, 20));
    bChangePedestrianColorNoTarget.addActionListener(
        new ActionSetPedestrianWithoutTargetColor(
            "Set Pedestrian without Target Color", model, pPedestrianColorNoTarget));
  }

  private void initColoringBySelfCategory(
      JComboBox<SelfCategory> cbSelfCategories,
      JPanel pSelfCategoryColor,
      JButton bChangeSelfCategoryColor) {
    cbSelfCategories.setSelectedIndex(0);

    SelfCategory selectedSelfCategory =
        cbSelfCategories.getItemAt(cbSelfCategories.getSelectedIndex());
    ...
    // When user changes a color, save it in the model.
    bChangeSelfCategoryColor.addActionListener(
        new ActionSetSelfCategoryColor(
            "Set Self Category Color", model, pSelfCategoryColor, cbSelfCategories));

    // Retrieve configured color from "model".
    cbSelfCategories.addActionListener(
        e -> {
          SelfCategory selectedSelfCategoryInner =
              cbSelfCategories.getItemAt(cbSelfCategories.getSelectedIndex());
          pSelfCategoryColor.setBackground(
              model.config.getSelfCategoryColor(selectedSelfCategoryInner));
        });
  }

  private void initColoringByHealthStatus(
      JButton bChangePedestrianColorLowerExposure,
      JPanel pPedestrianColorLowerExposure,
      JButton bChangePedestrianColorUpperExposure,
      JPanel pPedestrianColorUpperExposure,
      JButton bChangePedestrianColorInfectious,
      JPanel pPedestrianColorInfectious) {
    ...
    // When user changes a color, save it in the model.
    bChangePedestrianColorLowerExposure.addActionListener(
        new ActionSetPedestrianDefaultColor(
    ...
    // When user changes a color, save it in the model.
    bChangePedestrianColorUpperExposure.addActionListener(
        new ActionSetPedestrianExposedColor(
    ...
    // When user changes a color, save it in the model.
    bChangePedestrianColorInfectious.addActionListener(
        new ActionSetPedestrianInfectiousColor(
            "Set Pedestrian Infectious Color", model, pPedestrianColorInfectious));
  }

  private void initColoringByInformationState(
      JComboBox<InformationState> cbInformationStates,
      JPanel pInformationStateColor,
      JButton bChangeInformationStateColor) {

    cbInformationStates.setSelectedIndex(0);

    InformationState selectedInformationState =
        cbInformationStates.getItemAt(cbInformationStates.getSelectedIndex());
    ...
    // When user changes a color, save it in the model.
    bChangeInformationStateColor.addActionListener(
        new ActionSetInformationStateColor(
            "Set Information State Color", model, pInformationStateColor, cbInformationStates));

    // Retrieve configured color from "model".
    cbInformationStates.addActionListener(
        e -> {
          InformationState selectedInformationStateInner =
              cbInformationStates.getItemAt(cbInformationStates.getSelectedIndex());
          pInformationStateColor.setBackground(
              model.config.getInformationStateColor(selectedInformationStateInner));
        });
  ...
  private void initOtherSettingsPane(JLayeredPane otherSettingsPane) {
    otherSettingsPane.setBorder(
        BorderFactory.createTitledBorder(
            Messages.getString("SettingsDialog.additional.border.text")));

    FormLayout otherSettingsLayout =
        new FormLayout(
            createCellsWithSeparators(4), // col
    ...
    JCheckBox chInterpolatePositions =
        new JCheckBox((Messages.getString("SettingsDialog.chbInterpolatePositions.text")));
    JCheckBox chShowObstacles =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowObstacles.text")));
    JCheckBox chShowTargets =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowTargets.text")));
    JCheckBox chShowSources =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowSources.text")));
    JCheckBox chShowAbsorbingAreas =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowAbsorbingAreas.text")));
    JCheckBox chShowAerosolClouds =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowAerosolClouds.text")));
    JCheckBox chShowDropletClouds =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowDropletClouds.text")));
    JCheckBox chShowMeasurementAreas =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowMeasurementAreas.text")));
    JCheckBox chShowStairs =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowStairs.text")));
    JCheckBox chShowTargetChangers =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowTargetChangers.text")));
    JCheckBox chShowPedIds =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowPedestrianIds.text")));
    JCheckBox chShowPedestrianInOutGroup =
        new JCheckBox((Messages.getString("SettingsDialog.chbShowPedestrianInOutGroup.text")));
    JCheckBox chHideVoronoiDiagram =
        new JCheckBox((Messages.getString("SettingsDialog.chbHideVoronoiDiagram.text")));

    chInterpolatePositions.setSelected(model.config.isInterpolatePositions());
    chInterpolatePositions.addItemListener(
        e -> {
    ...
    chHideVoronoiDiagram.addItemListener(
        e -> {
    ...
    chShowObstacles.addItemListener(
        e -> {
    ...
    chShowAbsorbingAreas.addItemListener(
        e -> {
    ...
    chShowAerosolClouds.addItemListener(
        e -> {
    ...
    chShowDropletClouds.addItemListener(
        e -> {
    ...
    chShowMeasurementAreas.addItemListener(
        e -> {
    ...
    chShowTargetChangers.addItemListener(
        e -> {
    ...
    chShowPedestrianInOutGroup.addItemListener(
        e -> {
    ...
    otherSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblSnapshotDir.text") + ":"),
        cc.xy(2, row += NEXT_CELL));

    // Add text box to change snapshot directory
    JTextField tSnapshotDir =
        new JTextField(
            VadereConfig.getConfig().getString("SettingsDialog.snapshotDirectory.path", "."));
    ...
    final JButton bSnapshotDir =
        new JButton(Messages.getString("SettingsDialog.btnEditSnapshot.text"));
    bSnapshotDir.addActionListener(
        new ActionSetSnapshotDirectory("Set Snapshot Directory", model, tSnapshotDir, this));
    otherSettingsPane.add(bSnapshotDir, cc.xy(6, row));

    final JSpinner spinnerCellWidth = new JSpinner();
    final SpinnerNumberModel sModelCellWidth =
        new SpinnerNumberModel(
            model.config.getGridWidth(),
            model.config.getMinCellWidth(),
            model.config.getMaxCellWidth(),
            0.1);
    spinnerCellWidth.setModel(sModelCellWidth);

    spinnerCellWidth.addChangeListener(
        e -> {
    ...
    otherSettingsPane.add(
        new JLabel(Messages.getString("SettingsDialog.lblCellWidth.text") + ":"),
        cc.xy(2, row += NEXT_CELL));
   ...
   *
   * <ul>
   *   <li>Content columns are denoted with "pref". They are wide as the widest component (based on
   *       the component’s preferred size).
   *   <li>Columns are separated by a comma character.
   *   <li>"dlu" means dialog units. Dialog units are based on the pixel size of the dialog box
   *       font. They grow and shrink with the font and resolution. These elements represent thin
   *       separators. "dlu" elements are typically used as separator.
   * </ul>
   */
  private String createCellsWithSeparators(int totalCells, String prefix, String suffix) {
    ...
    String cells =
        Strings.repeat(cellWidth + cellSeparator + separatorWidth + cellSeparator, totalCells - 1);
    cells += cellWidth;
  ...
  private String createCellsWithSeparators(int totalCells) {
    ...
    String cellsWidthPrefixAndSuffix =
        createCellsWithSeparators(totalCells, separatorWidth, separatorWidth);
