...
public class VTable<D> extends JTable {
  ...
  public void setProjectViewModel(
      ProjectViewModel
          projectViewModel) { // TODO [priority=low] [task=refactoring] breaking mvc pattern?
        ...
        new MouseAdapter() {
          ...
          public void mouseClicked(MouseEvent event) {
            if (VTable.this.isEnabled() && SwingUtilities.isRightMouseButton(event)) {
              ...
              if (projectViewModel
                  != null) { // only the scenarioTable has a projectViewModel, not the outputTable
                if (projectViewModel.selectedScenariosContainChangedOnes(
                    VTable.this.getSelectedRows())) {
