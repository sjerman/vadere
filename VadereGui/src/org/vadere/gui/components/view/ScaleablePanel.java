...
public abstract class ScaleablePanel extends JPanel implements IScaleChangeListener {
  ...
  public void scaleChange(final double scale) {
    if (model.getTopography() != null && scrollPane.getMousePosition() != null) {
      ...
      if (mousePos.equals(
          lastMousePos)) { // if the mouse didn't move, use the previous zoom-target-point
      ...
      if (outOfViewportInWidth) {
        viewportMoveX =
            diffAbsMousePos
                .getX(); // + (keepFocusPoint && scaleChange > 1 ? diffToViewportCenterX * 0.2 : 0);
      ...
      if (outOfViewportInHeight) {
        viewportMoveY =
            diffAbsMousePos
                .getY(); // + (keepFocusPoint && scaleChange > 1 ? diffToViewportCenterY * 0.2 : 0);
      ...
      viewport.setViewPosition(
          new Point(
              (int) (viewport.getViewPosition().getX() + viewportMoveX),
  ...
  protected void paintComponent(Graphics g) {
    if (renderer != null && scrollPane != null) {
      ...
      renderer.render(
          (Graphics2D) g,
          (int) viewposition.getX(),
          (int) viewposition.getY(),
          viewport.getWidth(),
          viewport.getHeight());
    }
